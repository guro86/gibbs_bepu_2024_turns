...
metamodels = [
    ot.ParametricFunction(metamodel, [2, 4], [0.99, 1.0]) for metamodel in metamodels
]
...
samplers += [
    ot.RandomVectorMetropolisHastings(
        ...
        ot.Function(
            var_post_truncated_normal(
                dim=i, ndim=3, lb=lbs_var[i], ub=ubs_var[i], nexp=nexp
            )
        ),
    )
    for i in range(ndim)
...
for exp in range(nexp):
    logp = ot.Function(
        log_pdf_x(exp=exp, like=likes[exp], metamodel=metamodels[exp], ndim=ndim)
    )
